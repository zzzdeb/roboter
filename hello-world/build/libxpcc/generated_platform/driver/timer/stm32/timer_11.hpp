...
#ifndef XPCC_STM32_TIMER_11_HPP
#define XPCC_STM32_TIMER_11_HPP
 ...
 * {
 *     Timer11::resetInterruptFlags(Timer11::...);
 *
 *     ...
 * }
 * @endcode
 ...
 */
class Timer11 : public GeneralPurposeTimer
{
public:
	/// TypeId used to connect GPIO pins to this peripheral's Channel1.
	static const TypeId::Timer11Channel1 Channel1;
public:
	...
	setPeriod(uint32_t microseconds, bool autoApply = true)
	{
		...
		// unequal to zero)
		uint32_t cycles = microseconds * (SystemClock::Timer11 / 1000000UL);
...
public:
	...
	setInvertedPwm(uint32_t channel)
	{
...

		{
			...
			if (channel <= 1)
			{
				...
				flags <<= offset;
				flags |= TIM11->CCMR1 & ~(TIM_CCMR1_OC1M << offset);
				TIM11->CCMR1 = flags;
			}
			else {
				...
				flags <<= offset;
				flags |= TIM11->CCMR2 & ~(TIM_CCMR1_OC1M << offset);
	...
	{
		*(&TIM11->CCR1 + (channel - 1)) = value;
	}

	static inline Value
	getCompareValue(uint32_t channel)
	{
		return *(&TIM11->CCR1 + (channel - 1));
	}
...
public:
	...
	{
		TIM11->DIER |= static_cast<uint32_t>(dmaRequests);
	}
	...
	getInterruptFlags()
	{
		return InterruptFlag_t(TIM11->SR);
	}
