...
#ifndef XPCC_STM32_TIMER_5_HPP
#define XPCC_STM32_TIMER_5_HPP
 ...
 * {
 *     Timer5::resetInterruptFlags(Timer5::...);
 *
 *     ...
 * }
 * @endcode
...
public:
	...
	setPeriod(uint32_t microseconds, bool autoApply = true)
	{
		...
		// unequal to zero)
		uint32_t cycles = microseconds * (SystemClock::Timer5 / 1000000UL);
	...
	setInvertedPwm(uint32_t channel)
	{
...

		{
			...
			if (channel <= 1)
			{
				...
				flags <<= offset;
				flags |= TIM5->CCMR1 & ~(TIM_CCMR1_OC1M << offset);
				TIM5->CCMR1 = flags;
			}
			else {
				...
				flags <<= offset;
				flags |= TIM5->CCMR2 & ~(TIM_CCMR1_OC1M << offset);
...
public:
	...
	{
		TIM5->DIER |= static_cast<uint32_t>(dmaRequests);
	}
	...
	getInterruptFlags()
	{
		return InterruptFlag_t(TIM5->SR);
	}
