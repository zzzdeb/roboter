 ...
 * xpcc::IOStream in your main souce file, which will then be used to dump
...
namespace
{
	...
	static xpcc::BoundedQueue<ConfiguredTransaction, 8> queue;
	...
	callStarting()
	{
		...
		while ((I2C3->CR1 & I2C_CR1_STOP) and (deadlockPreventer-- > 0))
			;
		...
		if ((I2C3->SR2 & I2C_SR2_BUSY) and (nextOperation != xpcc::I2c::Operation::Restart))
		{
			// we wait a short amount of time for the bus to become free.
			deadlockPreventer = 10000;
			while ((I2C3->SR2 & I2C_SR2_BUSY) and (deadlockPreventer-- > 0))
				;
	...
	callNextTransaction()
	{
		if (!queue.isEmpty())
		{
			...
			uint_fast32_t deadlockPreventer = 100000;
			while (I2C3->CR1 & I2C_CR1_STOP && deadlockPreventer-- > 0)
				;
...
I2C3_EV_IRQHandler(void)
{
	...
	if (sr1 & I2C_SR1_SB)
	{
		...
		I2C3->DR = addressMode | (starting.address & 0xfe);
	}
...
//		(void) sr2;
	...
	else if (sr1 & I2C_SR1_TXE)
	{
		...
		if (writing.length > 0)
		{
			...
			I2C3->DR = *writing.buffer++; // write data
			--writing.length;
...
void
xpcc::stm32::I2cMaster3::initializeWithPrescaler(uint8_t peripheralFrequency, uint8_t riseTime, uint16_t prescaler)
{
...
// MARK: - ownership
bool
xpcc::stm32::I2cMaster3::start(I2cTransaction *transaction, ConfigurationHandler handler)
{
	xpcc::atomic::Lock lock;
